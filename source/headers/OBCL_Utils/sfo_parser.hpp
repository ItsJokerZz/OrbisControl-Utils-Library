// https://github.com/LightningMods/Itemzflow/blob/main/itemzflow/include/sfo.hpp
...
public:
    ...
    void printEntries()
    {
        for (u32 i = 0; i < hdr->entryCount; i++)
        {
            ...
            printf("SfoEntry[%d]: \"%20s\" fmt: 0x%X len: %d ", i, GetKey(e).c_str(), e->paramFmt, e->paramLen);
            if (isNum(e))
                printf("0x%08X", GetValue(e)); // u32(this->operator[](e)));
            else if (isText(e))
                printf("\"%s\"", GetString(e).c_str());
            else
                printf("ERROR unknown fmt?");
        }
    }
};
